...
Execution environment: grant network access before `scan`/`preflight`; restricted scans persist false backoff.

1. Inspect directly exposed and lazily discoverable tools in the current agent environment. If they support read-only marketplace search with full descriptions in search results or through a paired detail lookup, follow `prompts/discover_marketplaces_with_plugins.md` and pass successful source files with `--marketplace-results source=/tmp/file.json`; otherwise omit that option. Run `jobflow.py scan` and prune. Capture final `scan_run_id`, `found`, and `screening_job_ids`. If found is zero or IDs are empty, run reports and stop: report `accepted/rejected/deferred: none`, `generated PDFs: none`, and blockers; do not evaluate, generate, or deliver. Pasted JDs use `import-jd`. Require exact sponsor/alias matches; URLs refresh and two misses close a vacancy. Never apply, message, connect, or create accounts.
2. Run `jobflow.py jobs --status active --workflow-status screening --scan-run <scan_run_id>` and evaluate only returned IDs/files in order. Give a fresh reviewer `prompts/evaluate_job.md`, profile, master CV, job data, warnings, and verification items. Save `data/matches/<job-id>.json`; run `record-match`. Below 50 rejects; passes create `brief.json`.
